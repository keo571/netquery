"""
Interpreter node for Text-to-SQL agent.
...
def interpret_results_node(state: TextToSQLState) -> Dict[str, Any]:
    ...
    if state.get("execution_error"):
        ...
        data_table_html = df.to_html(
            classes="styled-table", index=False, border=0
        )
...
<p><strong>Agent:</strong> Text-to-SQL Agent</p>
...
"""
    ...
    final_response_html = markdown.markdown(final_response_str)
